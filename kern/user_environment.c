...
for (uint32 i = 0 ; i <USER_TOP; i+=(PAGE_SIZE*1024))
	...
	if(ptr_table!=NULL){
	    ...
	    uint32 *pp;
		frame_info_of_remove = get_frame_info(e->env_page_directory,(void*)ptr_table,&pp);
		//frame_info_of_remove->references--;
	//	if(frame_info_of_remove->references==0){
		...
		//pd_clear_page_dir_entry(e,i);
			...
			struct Frame_Info* frame_info_of_remove = NULL;
			uint32 *pp;
			frame_info_of_remove = to_frame_info(e->env_page_directory[i]);
			frame_info_of_remove->references--;
			if(frame_info_of_remove->references==0){
			free_frame(frame_info_of_remove);}
			e->env_page_directory[i]=0;
	...
	// [4] Free the page DIRECTORY from the main memory
			struct Frame_Info* frame_info_of_remove = NULL;
			uint32*pp=NULL;
			frame_info_of_remove = get_frame_info(e->env_page_directory,(void*)e->env_page_directory,&pp);
			frame_info_of_remove->references--;
			if(frame_info_of_remove->references==0){
			free_frame(frame_info_of_remove);}
	...
	//YOUR CODE ENDS HERE --------------------------------------------
/*	struct WorkingSetElement *temp_active;
		cprintf("\n active...........................%x\n",LIST_SIZE(&e->ActiveList));
			cprintf("\n second.........................%x\n",LIST_SIZE(&e->SecondList));
	LIST_FOREACH(temp_active, &(e->SecondList))
					{cprintf("\n second...........................%x\n",LIST_SIZE(&e->SecondList));

							LIST_REMOVE(&(e->SecondList), temp_active);
							unmap_frame(e->env_page_directory, (void*)temp_active->virtual_address);
							temp_active->empty = 1;

							uint32* ptr = NULL;
							get_page_table(e->env_page_directory, (void*)temp_active->virtual_address, &ptr);
							if (ptr != NULL)
								{cprintf("\n second table.....................\n");

							struct Frame_Info* frame_info_of_remove = NULL;
							frame_info_of_remove = to_frame_info((e->env_page_directory[PDX(temp_active->virtual_address)] >> 12) * PAGE_SIZE);
							frame_info_of_remove->references=0;
							free_frame(frame_info_of_remove);


							pd_clear_page_dir_entry(e, temp_active->virtual_address);
	...
	cprintf("\n active.....................\n");
		...
		uint32*ptr=NULL;
				struct Frame_Info* frame_info_of_remove = NULL;
				frame_info_of_remove = get_frame_info(e->env_page_directory,(void*)&e->ActiveList,&ptr);
				free_frame(frame_info_of_remove);




		 frame_info_of_remove = NULL;
		frame_info_of_remove = get_frame_info(e->env_page_directory,(void*)e->env_page_directory,&ptr);
		free_frame(frame_info_of_remove);
